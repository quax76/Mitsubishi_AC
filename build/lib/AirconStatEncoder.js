...
exports.encodeAirconStat = encodeAirconStat;
...
function encodeAirconStat(currentBytes, state) {
...
function currentReceiveData(bytes) {
    if (bytes.length < 21) {
...
function createCommandData(current, state) {
...
function createReceiveData(current, state) {
    const data = Buffer.alloc(18);
    data[5] = 0xff;
...
function preserveModelFlags(current, target, commandFrame) {
