...
exports.decodeAirconStat = decodeAirconStat;
const TemperatureTables_1 = require("./TemperatureTables");
const MODE_BY_CODE = {
    0: "auto",
    1: "dry",
    2: "cool",
...
function decodeAirconStat(bytes) {
    if (bytes.length < 21) {
...
function decodeMeasurements(bytes, start) {
    const result = {};
    for (let offset = start; offset + 3 < bytes.length - 2; offset += 4) {
        const type = bytes[offset];
        const subtype = bytes[offset + 1];
        if (type === 0x80 && subtype === 0x10) {
            result.outdoorTemperature = TemperatureTables_1.OUTDOOR_TEMPERATURES[bytes[offset + 2]];
        }
        else if (type === 0x80 && subtype === 0x20) {
            result.roomTemperature = TemperatureTables_1.INDOOR_TEMPERATURES[bytes[offset + 2]];
        }
