...
exports.localSubnetProbeAddresses = localSubnetProbeAddresses;
...
function mergeDevices(configured, discovered) {
    ...
    const normalizedMac = (mac) => mac?.toLowerCase().replace(/[^a-f0-9]/g, "");
    for (const device of discovered) {
        const mac = normalizedMac(device.mac);
        const index = merged.findIndex((entry) => entry.id === device.id ||
...
async function discoverViaSsdp(timeoutMs) {
...
async function discoverViaTlsCertificate(options, addresses) {
    const discovered = [];
    const ports = [51443, ...options.scanPorts.filter((port) => port !== 51443 && port > 1024)];
    await mapWithConcurrency(addresses, options.concurrency ?? 32, async (address) => {
...
function localSubnetProbeAddresses(seedAddresses = [], subnets = []) {
    const addresses = new Set();
    const prefixes = new Set();
    for (const networkInterface of Object.values(node_os_1.default.networkInterfaces())) {
...
async function readMhiTlsFingerprint(host, ports, timeoutMs) {
